...
class UserLogicController {
    ...
    private func loadMain(then handler: @escaping ViewStateHandler) {
        GitClient.standard.getUserPage(page: model.currentPage, perPage: 10) { response, error in
            if let error = error {
    ...
    private func loadFollowers(then handler: @escaping ViewStateHandler) {
        ...
        GitClient.standard.getUserFollowers(userLogin: parameters.0, page: model.currentPage, perPage: 10) { response, error in
            if let error = error {
                print(error.localizedDescription)
            } else {
                self.model.append(contentsOf: response)
    ...
    private func loadFollowing(then handler: @escaping ViewStateHandler) {
        ...
        GitClient.standard.getUserFollowing(userLogin: parameters.0, page: model.currentPage, perPage: 10) { response, error in
            if let error = error {
                print(error.localizedDescription)
            } else {
                self.model.append(contentsOf: response)
    ...
    private func loadStars(then handler: @escaping ViewStateHandler) {
        ...
        GitClient.standard.getRepositoryStars(fullName: parameters.0, page: model.currentPage, perPage: 10) { response, error in
            if let error = error {
                print(error.localizedDescription)
            } else {
                self.model.append(contentsOf: response)
    ...
    private func loadContributers(then handler: @escaping ViewStateHandler) {
        ...
        GitClient.standard.getRepositoryContributors(fullName: parameters, page: model.currentPage, perPage: 10) { response, error in
            if let error = error {
                print(error.localizedDescription)
            } else {
                self.model.append(contentsOf: response)
    ...
    private func loadMembers(then handler: @escaping ViewStateHandler) {
        ...
        GitClient.standard.getOrganizationMemebers(organizationLogin: parameters.0, page: model.currentPage, perPage: 10) { response, error in
            if let error = error {
                print(error.localizedDescription)
            } else {
                self.model.append(contentsOf: response)
